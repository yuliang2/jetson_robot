        ...
        with ThreadPoolExecutor() as executor:
            ...
            for name, motor_cmd in motor_cmds.items():
                ...
                if motor_instance is None:
                    continue
                future = executor.submit(motor_instance.sendrecv, motor_cmd)
                future_to_name[future] = name

            for future in as_completed(future_to_name):
                name = future_to_name[future]
                try:
                    motor_data_dict[name] = future.result()
                except Exception as e:
                    logger.warning(f"Failed to read motor data for {name}: {traceback.format_exc()}")
